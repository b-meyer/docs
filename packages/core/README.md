...
time via the `framework-ssg` CLI (bundled in this package) and hydrated as a SPA, with
...
**4. `tsconfig.json`** — extend the framework base, which provides all module shims (`*.vue`, `*.md`, `__FRAMEWORK_MERMAID__`) and shared compiler settings:

```json
{
  "extends": "@framework/core/tsconfig.base.json",
  "compilerOptions": {
    "paths": { "@/*": ["./src/*"] }
  },
  "include": ["src/**/*"]
...
- **Extra markdown-it plugins.** Pass additional plugins via `frameworkPlugin`'s options, or wrap the plugin array returned by `defineConfig`.
...
| `@framework/core/config`               | `FrameworkConfig`, `SidebarGroup`, `SidebarItem`, `CONFIG_KEY`                                                            |
| `@framework/core/vite`                 | `defineConfig(opts)` — the primary consumer entry point                                                                   |
| `@framework/core/ssg`                  | `createApp` — called automatically by the framework virtual entry; consumers do not call it directly                      |
| `@framework/core/sitemap`              | `filterPublicRoutes`, `buildSearchIndex`, `buildSitemap`, `patchCspScriptHash` — called by `framework-ssg`; **Node-only** |
...
| `@framework/core/client`               | TypeScript shims for `*.vue`, `*.md`, `__FRAMEWORK_MERMAID__`                                                             |
| `@framework/core/tsconfig.base.json`   | shared `tsconfig` base for consumer apps                                                                                  |
