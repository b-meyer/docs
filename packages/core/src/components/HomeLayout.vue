...
<template>
  <div class="mx-auto w-full max-w-[1152px] px-6 py-12 md:py-20">
    ...
    <section class="flex flex-col items-center gap-6 text-center md:gap-8">
      ...
      <div class="space-y-3">
        <h1 v-if="hero.name" class="text-4xl font-bold tracking-tight text-primary-500 md:text-6xl">
          {{ hero.name }}
        </h1>
      ...
      <div v-if="hero.actions?.length" class="flex flex-wrap items-center justify-center gap-3">
        <RouterLink
          ...
          :class="[
            ...
            action.theme === 'alt'
              ? 'border border-gray-300 text-gray-900 hover:border-primary-400'
    ...
    <section v-if="features.length" class="mt-14 flex flex-wrap justify-center gap-4 md:mt-20">
      ...
      >
        <div
          v-if="feature.icon"
          class="mb-4 inline-flex size-11 items-center justify-center rounded-lg bg-primary-50 text-2xl"
          aria-hidden="true"
        >
          {{ feature.icon }}
        </div>
