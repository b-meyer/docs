...
/**
 * Preprocesses raw markdown, replacing `<<< ./path/to/file` lines with fenced
 * code blocks. Supports optional `#region-name` suffix to extract a named region.
 *
 * HMR limitation: snippet source files are not watched in dev mode. Changes
 * require a manual page reload or saving the importing markdown file.
 */
