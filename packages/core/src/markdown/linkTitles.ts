...
/**
 * Core-rule half of automatic link-title resolution: when a link's display text
 * is the bare filename (e.g. `[YinYang.md](YinYang.md)`), replace it with the
 * sidebar title supplied by `getTitles`. Custom display text is left untouched.
 * Runs before rendering so it composes cleanly with `mdLinkRewriter`.
 */
