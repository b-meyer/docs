...
const ENTRY = 'virtual:framework-entry';
const TEMP_DIR = '.framework-ssg-temp';
...
async function buildClientBundle(root: string, outDir: string): Promise<void> {
  console.info('[framework-ssg] Building for client...');
  await build({
    root,
    build: {
      outDir,
...
async function buildServerBundle(root: string, tempDir: string): Promise<void> {
  console.info('[framework-ssg] Building for server...');
...
async function runBuild(): Promise<void> {
  ...
  const frameworkConfig = (viteConfig as Record<string, unknown>)[
    '__frameworkConfig'
  ...
  console.info(`[framework-ssg] Rendering ${renderedPaths.length} pages...`);

  const base = viteConfig.base ?? '/';
  const { ssrManifest, mainJs, mainCss } = loadBuildArtifacts(outDir, base);
  const staticHead = buildStaticHead(frameworkConfig, base);
  ...
  console.info('[framework-ssg] Build finished.');
}
...
function buildStaticHead(config: FrameworkConfig | null, base: string): string {
...
function minifyHtml(html: string): string {
  ...
  const out = safe
    .replaceAll(/\s+/gu, ' ')
    .replaceAll('> <', '><')
    .replaceAll(/<!--(?!\s*\[if|\s*<!\[endif)[\s\S]*?-->/gu, '')
    .trim();
...
try {
  ...
  console.error('[framework-ssg] Build failed:', err);
  process.exit(1);
}
