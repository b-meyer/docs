...
// SSR-safe defaults. Reconciled to DOM/localStorage state via syncTheme()
// (called from App.vue onMounted), keeping the server render and the initial
// client render identical so Vue hydration never sees a mismatch on theme state.
// The pre-paint script already applies .dark and CSS vars before Vue boots, so
// there is no visual flash even though the reactive state starts at defaults.
