...
import { syncTheme, useTheme } from './useTheme';
...
describe('useTheme', () => {
  ...
  it('syncTheme reads dark mode from <html> classList', () => {
  ...
  it('syncTheme reads light mode when .dark is absent', () => {
    syncTheme();
    const { theme } = useTheme();
    expect(theme.value).toBe('light');
  });

  it('syncTheme reads hue from localStorage', () => {
    localStorage.setItem('brand-hue', '180');
    syncTheme();
    const { hue } = useTheme();
    expect(hue.value).toBe(180);
  });

  it('syncTheme reads intensity from localStorage', () => {
    localStorage.setItem('brand-intensity', '75');
    syncTheme();
