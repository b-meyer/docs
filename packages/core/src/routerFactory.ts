...
import type { FrameworkConfig, SidebarGroup, SidebarItem } from './config';
 ...
 * - Record config: longest-prefix match on `routePath` (e.g. '/guide/intro' matches
 *   '/guide/' before '/'). Falls back to an empty array if no prefix matches.
 */
export function resolveSidebar(config: FrameworkConfig, routePath: string): SidebarGroup[] {
  ...
  const matched = keys
    .filter((k) => {
      ...
      return routePath === k || routePath.startsWith(prefix) || k === '/';
    })
...
/**
 * Returns the full route path without its leading slash, normalised as the item
 * identity key used throughout the nav layer.
 *
 *   '/'            → 'index'
 *   '/YinYang'     → 'YinYang'
...
export function buildFlatOrder(config: FrameworkConfig, routePath: string): SidebarItem[] {
