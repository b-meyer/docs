...
const mockConfig: QDocsConfig = { title: 'Test Site', sidebar: [] };
...
describe('useConfig', () => {
  it('returns the provided QDocsConfig', () => {
  ...
  it('throws a descriptive error when CONFIG_KEY is not provided', () => {
    expect(() => mount(ComponentThrowingWithoutConfig)).toThrow('no QDocsConfig');
  });
});
