...
import { mdLinkTitles } from './markdown/linkTitles.ts';
...
export type QDocsPluginOptions = Pick<QDocsConfig, 'markdown'> & {
  /**
   * Absolute path to the consumer's pages directory. When provided, the dev server serves `GET
   * /search-index.json` on demand so that the lazy-fetch search works in dev without a prior build
   * step. Derived from rootDir when omitted (set automatically by defineConfig).
   */
  pagesDir?: string;
};

// Module-level state set by defineConfig before Vite processes plugins.
let qdocsConfigStore: QDocsConfig | null = null;
let rootDir: string = process.cwd();
let resolvedBase: string = '/';
let detectedRoutesFolder = 'src'; // updated by the config() hook once userConfig.root is known

export function setQDocsConfig(config: QDocsConfig): void {
  qdocsConfigStore = config;
}
...
const VIRTUAL_CONFIG_ID = 'virtual:@qdocs/config';
const RESOLVED_VIRTUAL_CONFIG_ID = '\0virtual:@qdocs/config';

const VIRTUAL_ENTRY_ID = 'virtual:qdocs-entry';
const RESOLVED_VIRTUAL_ENTRY_ID = '\0virtual:qdocs-entry';

const VIRTUAL_HEADER_END_ID = 'virtual:@qdocs/header-end';
const RESOLVED_VIRTUAL_HEADER_END_ID = '\0virtual:@qdocs/header-end';
...
export function buildPrePaintScript(config: QDocsConfig | null): string {
...
function virtualModulesPlugin(): Plugin {
  return {
    name: 'qdocs:virtual-modules',
    resolveId(id) {
      if (id === VIRTUAL_CONFIG_ID) return RESOLVED_VIRTUAL_CONFIG_ID;
      if (id === VIRTUAL_ENTRY_ID) return RESOLVED_VIRTUAL_ENTRY_ID;
      if (id === VIRTUAL_HEADER_END_ID) return RESOLVED_VIRTUAL_HEADER_END_ID;
    },
    load(id) {
      if (id === RESOLVED_VIRTUAL_CONFIG_ID) {
        if (!qdocsConfigStore)
          throw new Error('qdocs: defineConfig must be called before this virtual module resolves');
        return `export default ${JSON.stringify(qdocsConfigStore)};`;
      }
      if (id === RESOLVED_VIRTUAL_ENTRY_ID) {
        return [
          `import { createApp } from 'qdocs/ssg';`,
          `export { createApp };`,
          `if (!import.meta.env.SSR) { createApp(); }`,
        ].join('\n');
      }
      if (id === RESOLVED_VIRTUAL_HEADER_END_ID) {
        const comp = qdocsConfigStore?.headerEnd;
        if (!comp) return 'export default null;';
        const abs = pathResolve(rootDir, comp).replaceAll('\\', '/');
        return `import HeaderEnd from ${JSON.stringify(abs)};\nexport default HeaderEnd;`;
      }
    },
    configResolved(cfg) {
      rootDir = cfg.root;
      resolvedBase = cfg.base ?? '/';
      // Expose qdocs config so build.ts can read it after resolveConfig().
      (cfg as Record<string, unknown>)['__qdocsConfig'] = qdocsConfigStore;
    },
  };
}
...
function buildConfigMetaTags(config: QDocsConfig): HtmlTagDescriptor[] {
 ...
 */
export function buildHeadTags(config: QDocsConfig | null, base: string): HtmlTagDescriptor[] {
...
<html lang="en">
  <head>
    <script type="module" src="/@id/virtual:qdocs-entry"></script>
  </head>
  <body>
    <div id="app"></div>
  </body>
...
function injectHtmlPlugin(): Plugin {
  return {
    name: 'qdocs:inject-html',
    transformIndexHtml(html) {
      const patched = /<body(?![^>]*class)/iu.test(html)
        ? html.replace(/<body/iu, `<body class="${BODY_CLASS}"`)
        : html;
      return { html: patched, tags: buildHeadTags(qdocsConfigStore, resolvedBase) };
    },
  };
}
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      server.middlewares.use((req, res, next) => {
        ...
        // Skip static asset extensions only — not arbitrary dotted path segments like
        // Azure DevOps project names (e.g. /repos/devops/CIT.Platform).
        const ASSET_EXT =
          /\.(?:js|ts|css|svg|png|jpg|jpeg|gif|webp|ico|woff2?|ttf|eot|map|json|mjs|cjs|vue|md)(?:\?.*)?$/iu;
        // Also serve the app shell for /index.html — SWA CLI rewrites dotted-path routes
        // (e.g. /repos/devops/CIT.Platform) to /index.html and may not forward Accept: text/html.
        const isSwaRewriteTarget = url === '/index.html';
        if (
          !isSwaRewriteTarget &&
          (!accept.includes('text/html') || (url !== '/' && ASSET_EXT.test(url)))
        ) {
          next();
          return;
        }
...
/**
 * The qdocs single Vite-plugin entry. Consumers do: `plugins: [qdocsPlugin({ markdown: { mermaid:
 * true } })]` Markdown-it plugin order is load-bearing: anchor → linkRewriter → tableWrapper →
 * mermaid → containers → alerts.
 */
export function qdocsPlugin(options: QDocsPluginOptions = {}): PluginOption[] {
      ...
      async markdownItSetup(md) {
        ...
        md.use(mdLinkTitles, () => {
          ...
          for (const item of items) {
            if (item.path !== undefined) map.set(item.path, item.title);
          }
          return map;
        });
