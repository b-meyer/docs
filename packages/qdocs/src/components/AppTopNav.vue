...
<template>
  <nav v-if="config.nav?.length" class="hidden items-center md:flex">
      ...
      <PopoverRoot v-else-if="item.items">
        ...
        <PopoverPortal>
          <PopoverContent
            :side-offset="18"
            align="start"
            class="dark:bg-gray-125 z-50 min-w-[180px] rounded-lg border border-gray-200 bg-white py-1.5 shadow-lg"
          >
            <template v-for="(group, gi) in item.items" :key="group.text ?? gi">
              ...
              <template v-for="child in group.items" :key="child.text">
                <a
                  ...
                  rel="noopener noreferrer"
                  class="hover:bg-primary-50 dark:hover:bg-gray-175 flex items-center justify-between px-3 py-1.5 text-sm text-gray-700 hover:text-gray-900 dark:text-gray-700 dark:hover:text-gray-900"
                >
                  {{ child.text }}
                  <svg
                ...
                <RouterLink
                  ...
                  :to="child.link"
                  class="hover:bg-primary-50 dark:hover:bg-gray-175 block px-3 py-1.5 text-sm text-gray-700 hover:text-gray-900 dark:text-gray-700 dark:hover:text-gray-900"
                >
                  {{ child.text }}
                </RouterLink>
              </template>
