...
// SSR doesn't need the search index — search is a client-only UI. Skipping the
// eager glob server-side keeps each rendered route's HTML free of the index
// payload and avoids parsing every .md twice (server + client).
const raw: Record<string, string> = import.meta.env.SSR
  ? {}
